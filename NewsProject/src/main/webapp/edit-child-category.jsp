...
<body>
    <div class="container">
        ...
        <form action="manage-category" method="post" accept-charset="UTF-8">
		    <input type="hidden" name="action" value="edit-child-category"/>
		    <input type="hidden" name="childCategoryId" value="${category.categoryId}"/>
		    <div class="row mb-3">
		        <div class="col-6">
		            ...
		            <select class="form-control" id="parent" name="parent">
		                <c:forEach var="parentCategory" items="${parentCategories}">
		                    <option value="${parentCategory.categoryId}"
									<c:if test="${parentCategory.categoryId == category.parent.categoryId}">selected</c:if>
                                                            >${parentCategory.description}</option>
		                </c:forEach>
		            </select>
		        </div>
		    </div>
